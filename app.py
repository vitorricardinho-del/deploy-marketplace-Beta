...
def eh_o_vitor_logado():
    # 1. O Flask-Login verifica se tem alguém logado no app agora
    if current_user.is_authenticated:
        
        # 2. Pega o e-mail logado, padroniza (letras minúsculas e sem espaços)
        email_atual = current_user.email.lower().strip()
        
        # 3. Transforma o e-mail atual em uma Hash SHA-256 digital
        hash_atual = hashlib.sha256(email_atual.encode('utf-8')).hexdigest()
        
        # 🔒 O HASH SECRETO DO SEU E-MAIL (Ninguém descobre o e-mail olhando para isso!)
        HASH_DO_VITOR = "62cb801d6d053699405774827ee01e8a2d6de446f05fdc61861004b09dff7923"
        
        # 4. Compara as duas assinaturas digitais
        if hash_atual == HASH_DO_VITOR:
            return True
...
def limpar_expirados():
    ...
    # Removemos os limites manuais de 7, 15 e 30 dias porque o banco já tem a data pronta
    try:
        # 1. No select, agora buscamos a 'data_expiracao'
        ...
        for p in pedidos_nuvem:
            try:
                # 2. Transformamos a 'data_expiracao' do banco em data do Python
                exp_str = p.get('data_expiracao', '').replace('T', ' ')[:19] 
                if not exp_str:
                    continue # Se não houver data, ignora para não dar erro
            ...
            # 3. MUDANÇA AQUI: Se a data de hoje for maior que o vencimento, ele vai para a lista
            if hoje > data_vencimento:
                vencidos.append(p)

        # 4. Sua lógica de remoção do HD e do Banco continua IGUALZINHA abaixo
        for p in vencidos:
...
def salvar_pedido():
    ...
    try:
            ...
            # AGORA É REGRA GERAL: 365 dias para todo mundo!
            ...
            dados_pedido = {
                ...
                "data_expiracao": vencimento.isoformat(), # Todo anúncio nasce com 1 ano
                "plano": 0, 
...
def completar_perfil():
    if request.method == 'POST':
        ...
        # 2. Lógica da Foto de Perfil (Mantida)
...
# --- GET (Visualização da página - Estilo Mural) ---
    try:
        try:
            agora = datetime.now(fuso_ivinhema)
        except:
        ...
        # O SEGREDO ESTÁ AQUI: Transformar o ID de texto para NÚMERO (int)
        meu_id_numero = int(current_user.id)

        # 1. Busca os dados no Supabase filtrando pelo ID (agora como número!)
        ...
        # 2. Transforma os dicionários em Objetos Pedido
        meus_anuncios = []
        for p in dados_pedidos:
            try:
                obj_pedido = Pedido(**p)
                
                # 🔥 O CÁLCULO DOS 30 DIAS DO TURBO 🔥
                if not hasattr(obj_pedido, 'data_expiracao') or obj_pedido.data_expiracao is None:
                    
                    # 1. Tenta pegar a data de postagem original do anúncio (se falhar, usa o 'agora')
                    data_inicio = getattr(obj_pedido, 'data_postagem', agora)
                    
                    # 2. MÁGICA: Soma exatos 30 dias para a expiração!
        ...
        meus_anuncios = []
        agora = datetime.now() # Fallback caso fuso_ivinhema falhe
...
@precisa_de_senha
def admin_mural():
    if not eh_o_vitor_logado():
        print("🚨 TENTATIVA DE INVASÃO: Alguém tentou acessar o admin sem a conta do Vitor!")
        return redirect(url_for('exibir_mural'))
    limpar_expirados()
    
    try:
        # =========================================================================
        # NOVA BUSCA: Parceiros do Food Online aguardando aprovação (Invisíveis no App)
        # Puxa todas as lojas cujo 'aprovado' seja False
        # =========================================================================
        ...
        # 1. Busca todos os pedidos para o painel (Sua lógica original mantida 100%)
        # Colocamos o acento no "usuário" para o Supabase achar a tabela
        ...
        # 2. Busca estatísticas de vendas (Sua lógica original mantida 100%)
        ...
        # 3. Recriando o stats_categorias (Group By) (Sua lógica original mantida 100%)
        contagem = {}
        ...
        # 4. Busca os mais acessados (Sua lógica original mantida 100%)
        ...
        # Retorna o seu HTML original injetando a nova variável 'parceiros_food' sem quebrar as outras
        return render_template(
            ...
            parceiros_food=parceiros_pendentes  # <--- NOVA VARIÁVEL ENVIADA PARA O HTML
        )
...
def admin_aprovar_loja(loja_id):
    ...
    try:
        # Atualiza o status da lanchonete para True no Supabase
        supabase.table("categorias").update({"aprovado": True}).eq("id", loja_id).execute()
        
        # Redireciona de volta para o painel admin atualizando a página
...
def tornar_premium(id):
    ...
    try:
        ...
        # Preparando as datas para o caso de aprovação (Regra dos 365 dias)
        agora = datetime.now(fuso_ivinhema)
        nova_expiracao = agora + timedelta(days=30)

        # 2. LÓGICA DE APROVAÇÃO (Ajustada para focar no OURO)
        dados_update = {}

        if p.plano == 0 and p.plano_aguardando > 0:
            # APROVAÇÃO: Transforma em Ouro (2), joga pro topo e renova 1 ano
            p.plano = 2 
            p.is_premium = True
            ...
            # O FREIO: Inverte o status premium e ajusta o plano junto!
            novo_status = not p.is_premium
            
            dados_update = {
                "is_premium": novo_status,
                # A MÁGICA AQUI: Se novo_status for True, vira 2. Se for False, vira 0 (Gratuito)
                "plano": 2 if novo_status else 0 
            }
...
@precisa_de_senha # Ou @login_required se for o próprio usuário
def impulsionar_ouro(id):
    ...
    try:
        # O PULO DO GATO: Plano vira 2, sobe no 'data_criacao' e ganha +1 ano
        supabase.table("pedido").update({
            "plano": 2,
            "is_premium": True,
            "data_criacao": agora.isoformat(),
            "data_expiracao": nova_expiracao.isoformat(),
            "plano_aguardando": 0 # Limpa a fila de espera
        }).eq("id", id).execute()
...
def solicitar_turbo(id):
    try:
        # 1. Verifica se o anúncio realmente pertence ao usuário logado
        ...
        # 2. Marca o anúncio como "Aguardando Ouro" (Plano 2)
        # Isso faz ele aparecer no seu painel Admin como "PIX Pendente"
        supabase.table("pedido").update({
            "plano_aguardando": 2
        }).eq("id", id).execute()
...
# 🔑 O SEGREDO: Faz o Flask aceitar tanto o ID puro quanto o ID com o nome do produto no final!
@app.route('/anuncio/<int:id>', defaults={'slug': ''})
@app.route('/anuncio/<int:id>/<string:slug>')
def ver_anuncio_unico(id, slug=""):
    try:
        # 1. Busca o pedido e já traz os dados do autor (Lógica original mantida)
        ...
        # 🔗 ACOPLAMENTO ADAPTADOR: Corrige as variações da coluna preço
        if 'preço' in dados:
            dados['preco'] = dados['preço']
        elif 'price' in dados:
            dados['preco'] = dados['price']

        # Preenche os dados da classe sem dar erro de argumento
        ...
        # --- CLASSE ANUNCIOFAKE TOTALMENTE BLINDADA ---
        class AnuncioFake:
            def __init__(self, original):
                ...
                # Captura a foto local da pasta static/uploads
                ...
                # 🔄 O PULO DO GATO CONTRA O ERRO DE DATA:
                # Se a data vier como texto (str), transformamos em objeto datetime para o strftime não travar!
                bruto_data = getattr(original, 'data_criacao', getattr(original, 'data_postagem', None))
                if isinstance(bruto_data, str):
                    try:
                        # Tenta converter removendo o fuso horário se houver
                        limpo_data = bruto_data.split('.')[0].replace('Z', '')
                        self.data_postagem = datetime.strptime(limpo_data, "%Y-%m-%dT%H:%M:%S")
                    except:
                        try:
                            self.data_postagem = datetime.strptime(bruto_data.split(' ')[0], "%Y-%m-%d")
                        except:
                            self.data_postagem = datetime.now()
                else:
                    self.data_postagem = bruto_data if bruto_data else datetime.now()
        ...
        # Retorna injetando o anúncio único para abrir os detalhes na tela
        return render_template('mural.html', 
