...
def salvar_pedido():
    ...
    # 4. Salvar no Banco e Redirecionar (Onde a página caía)
    try:
...
def cadastro_usuario():
    if request.method == 'POST':
        ...
        if len(senha) < 8:
            # Se a senha for curta, ele para aqui e avisa o erro
            return "Erro: A senha precisa de pelo menos 8 dígitos!", 400
        try:
...
def ver_anuncio_unico(id):
    # 1. Busca o anúncio original no banco
    p_real = Pedido.query.get_or_404(id)
    
    # 2. Criamos uma versão "Exibição" que fura o bloqueio do HTML
    # O banco de dados NÃO é alterado aqui, é só para essa tela
    class AnuncioFake:
        def __init__(self, original):
            ...
            self.foto2 = original.foto2
            self.foto3 = original.foto3
            self.data_criacao = original.data_criacao
            self.preco = original.preco
            self.acessos = original.acessos
            # O SEGREDO: Aqui a gente força o HTML a mostrar
            self.is_premium = True 
            self.plano = 2 # Garante que apareça o selo ou destaque

    anuncio_para_exibir = AnuncioFake(p_real)
    
    # 3. Manda para o seu mural.html de sempre
