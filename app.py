...
import random # Necessário para recriar o seu func.random()
...
def exibir_mural():
    ...
    if current_user.is_authenticated:
        ...
        try:
            # O parâmetro count="exact" faz o Supabase contar os itens sem precisar baixar os dados
    ...
    try:
        # AQUI O SEGREDO: O select deve pedir os dados do autor também
        ...
        # Transforma em objetos e vincula o Autor corretamente
        pedidos_obj = []
        for p in dados_pedidos:
            obj_pedido = Pedido(**p)
            if p.get('autor'):
                obj_pedido.autor = Usuario(**p['autor']) # Vincula os dados do autor
            pedidos_obj.append(obj_pedido)
        
        # Transforma os dicionários do Supabase na nossa classe Pedido para não quebrar o seu HTML
        pedidos_obj = [Pedido(**p) for p in dados_pedidos]
        
        # --- RECRIANDO SUA LÓGICA DO func.random() ---
        agrupados_por_plano = {}
        ...
        pedidos = []
        # Pega do Plano 2, depois 1, depois 0
        for plano in sorted(agrupados_por_plano.keys(), reverse=True):
            lista = agrupados_por_plano[plano]
            random.shuffle(lista) # Embaralha os anúncios que têm o mesmo plano
